...
class AzureStorage {
  ...
  static final String sharedAccessSignature = 'SharedAccessSignature';
...
//    print('get path: $path'); //DEBUG LOG
  ...
  Future<List<dynamic>> filterTableRows({
    ...
    String selectParams = _resolveNodeParams(fields);
    String path =
        'https://${config[accountName]}.table.core.windows.net/$tableName?\$filter=${filter.isEmpty ? '*' : filter}&\$select=$selectParams&\$top=$top';
//    print('path to upload: $path'); //DEBUG LOG
  ...
  Future<Map<String, dynamic>> filterNextTableRows({
    required String tableName,
    String? filter,
    int top = 20,
    List<String>? fields,
    ...
    String selectParams = _resolveNodeParams(fields);
    String path =
        'https://${config[accountName]}.table.core.windows.net/$tableName?';
    if (filter != null && filter.isNotEmpty) {
      path += '\$filter=$filter&';
    }
    // path += '\$select=$selectParams&\$top=$top';
    path += '\$top=$top';
    // String path =
    //     'https://${config[accountName]}.table.core.windows.net/$tableName?\$filter=$filter&\$select=$selectParams&\$top=$top';
    if (nextRowKey != null &&
        nextPartitionKey != null &&
        nextRowKey.isNotEmpty &&
        nextPartitionKey.isNotEmpty) {
      path += '&NextPartitionKey=$nextPartitionKey&NextRowKey=$nextRowKey';
    }
//    print('path to upload: $path'); //DEBUG LOG
    var request = http.Request('GET', Uri.parse(path));
    request.headers['Accept'] = 'application/json;odata=nometadata';
