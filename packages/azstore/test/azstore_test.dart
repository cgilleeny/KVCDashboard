...
void main() {
  group('Azure Tables Tests', () {
    ...
    test('Filter next table rows', () async {
      var storage = AzureStorage.parse(connectionString);

      // populate table with 9 rows
      for (var i = 0; i < 9; i++) {
        Map<String, dynamic> rowMap = {
          "Address": "Santa Clara",
          "Age": 23 + i,
      ...
      Map<String, dynamic> result =
          await storage.filterNextTableRows(tableName: testTableName, top: 5);
      expect(result["Rows"].length, 5);

      result = await storage.filterNextTableRows(
          tableName: testTableName,
          top: 5,
